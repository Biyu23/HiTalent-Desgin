 ...
 * 对需求文本进行匹配打分
 */
function matchRequirement(requirement: string): {
  bestMatch: string;
  confidence: 'high' | 'medium' | 'low';
  reason: string;
  alternatives: Array<{ name: string; reason: string }>;
 ...
 * 根据组件文档生成代码骨架模板
 */
function generateSkeleton(doc: ComponentDoc): string {
  switch (doc.name) {
    case 'Button':
 ...
 * 根据匹配到的组件和需求生成代码指引
 */
function buildCodeGuide(
  doc: ComponentDoc,
  requirement: string,
): GenerateCodeResult['codeGenerationGuide'] {
  const lower = requirement.toLowerCase();
 ...
 */
export function handleGenerateCode(
  input: GenerateCodeInput,
): GenerateCodeResult {
  // 如果未指定组件名，自动匹配
  let targetName = input.componentName;
  let matchInfo: ReturnType<typeof matchRequirement> | null = null;
